...
const AddProductCategory = props => {
    ...
    const initList = () => {
        if (props.categories.length > 0) {
            ...
            ncl.push(<SelectCategory separator='&#8250;' lvl={props.categories[props.categories.length -1].id} parent={props.categories[props.categories.length -1]} csrftoken={props.csrftoken} setCategory={setCategory} />)
            setCategoriesList(ncl)
        } else {
            let select = (<SelectCategory separator='&#8250;' lvl={0} parent={null} csrftoken={props.csrftoken} setCategory={setCategory} />)
            let d = (
                    <div className="h-auto w-auto bg-gray-300" >
                        <SelectCategory separator='&#8250;' lvl={0} parent={null} csrftoken={props.csrftoken} setCategory={setCategory} />
                    </div>
                )
            setCategoriesList(d)
        }
