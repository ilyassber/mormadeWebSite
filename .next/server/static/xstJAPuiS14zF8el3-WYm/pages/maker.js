...
// CONCATENATED MODULE: ./components/graphics/buttons/BtnTtb.js
var BtnTtb_jsx = external_react_default.a.createElement;
...
/* harmony default export */ var buttons_BtnTtb = (BtnTtb);
...
const Sidebar = props => {
  ...
  const btnsList = props.data.map(function (value) {
    if (value.id == clicked) {
      return SideBar_jsx(buttons_BtnTtb, {
...
// EXTERNAL MODULE: ./components/graphics/buttons/index.js + 2 modules
...
function addProduct(data, csrttoken) {
  return new Promise((resolve, reject) => {
    ...
    external_axios_default.a.post('http://localhost:8000/products/', external_querystring_default.a.stringify(content), axiosConfig).then(response => {
      resolve(response.data);
    }).catch(error => {
      reject(error);
    });
...
const SelectCategory = props => {
  const optionsRef = Object(external_react_["useRef"])();
  ...
  let content = SelectCategory_jsx("div", {
    className: props.className
  }, SelectCategory_jsx("div", {
    className: "border border-1 p-1"
  }, SelectCategory_jsx("input", {
...
const UserPage = props => {
  ...
  Object(external_react_["useEffect"])(() => {
    ...
    if (props.cookies.utoken != null) {
      external_axios_default.a.get('http://localhost:8000/products/').then(res => {
