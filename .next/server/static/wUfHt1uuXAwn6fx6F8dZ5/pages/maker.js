...
function getCategories(lvl, id, csrttoken) {
  return new Promise((resolve, reject) => {
    ...
    external_axios_default.a.defaults.withCredentials = true;
    external_axios_default.a.post("http://localhost:8000" + '/api/categories/', external_querystring_default.a.stringify(content), axiosConfig).then(response => {
      resolve(response.data);
    }).catch(error => {
      reject(error);
    });
...
const UserPage = props => {
  ...
  if (!isLoading && data && data.length > 0) {
    ...
    content = maker_jsx("div", {
      ...
      cookies: props.cookies,
      className: "w-full"
    }), maker_jsx(WelcomeBar["a" /* default */], {
      className: "pt-12"
    }), maker_jsx("div", {
